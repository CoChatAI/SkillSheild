export interface FullClawHubScrapeScriptOptions {
  baseUrl?: string;
  wait?: boolean;
  limit?: number;
  useLlm?: boolean;
...
export async function runFullClawHubScrape(options: FullClawHubScrapeScriptOptions = {}) {
  const fetchImpl = options.fetchImpl ?? fetch;
  const endpoint = buildFullClawHubScrapeUrl(options);
...
export function buildFullClawHubScrapeUrl(options: Omit<FullClawHubScrapeScriptOptions, 'fetchImpl'> = {}) {
  const url = new URL('/scrape/clawhub', options.baseUrl ?? 'http://localhost:3100');
...
function parseCliArgs(args: string[]): FullClawHubScrapeScriptOptions {
  const options: FullClawHubScrapeScriptOptions = {};
